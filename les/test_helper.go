...
import (
	...
	"odinchain/accounts/abi/bind/backends"
	"odinchain/consensus/ethash"
	"odinchain/contracts/checkpointoracle/contract"
	"odinchain/core"
	"odinchain/core/rawdb"
	"odinchain/core/types"
	"odinchain/odin"
	"odinchain/les/flowcontrol"
	"odinchain/light"
	"odinchain/params"
)
...
func testIndexers(db ethdb.Database, odr light.OdrBackend, config *light.IndexerConfig) []*core.ChainIndexer {
	...
	indexers[1] = odin.NewBloomIndexer(db, config.BloomSize, config.BloomConfirms)
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db ethdb.Database, peers *peerSet, ulcServers []string, ulcFraction int) *clientHandler {
	...
	client := &LightEthereum{
		lesCommons: lesCommons{
			...
			config:      &odin.Config{LightPeers: 100, NetworkId: NetworkId},
