...
type Service struct {
	...
	eth    *odin.Ethereum     // Full Ethereum service if monitoring a full node
...
func New(url string, ethServ *odin.Ethereum, lesServ *les.LightEthereum) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*odin.NodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", odin.ProtocolVersions[0])
	} else {
