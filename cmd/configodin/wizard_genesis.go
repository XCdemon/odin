...
func (w *wizard) makeGenesis(chainId uint64) {
	...
	fmt.Println()
	fmt.Println("Which account should be used as coinbase?")
	// Read the address of the account to fund
	if address := w.readAddress(); address != nil {
		genesis.Config.Clique.Coinbase = *address
	}
	genesis.Config.ChainID = new(big.Int).SetUint64(chainId)

	// All done, store the genesis and flush to disk
	log.Info("Configured new genesis block", "chainId", chainId)
	...
	gethJson := filepath.Join(".", fmt.Sprintf("%s.json", w.network))
